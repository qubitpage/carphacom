...
function verifyResetToken(token: string): { email: string; actor_type: string } | null {
  try {
    ...
    return { email: data.entity_id || data.email, actor_type: data.actor_type || 'user' }
  } catch {
    return null
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    console.log(`[Reset Password] Password updated for ${decoded.email} (${decoded.actor_type})`)
