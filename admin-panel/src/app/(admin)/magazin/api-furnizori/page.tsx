...
function formatDate(iso: string | null): string {
  ...
  try {
    ...
    let ago = ''
    if (mins < 1) ago = 'acum cateva secunde'
...
function cronToText(cron: string): string {
  ...
  if (cron === '0 * * * *') return 'La fiecare ora'
...
export default function ApiFurnizoriPage() {
  ...
  const handleRefreshToken = async () => {
    ...
    try {
      ...
      if (json.success) {
        setMessage({ type: 'success', text: `Token reinnoit cu succes! Expira: ${json.expires_at}` })
        setCredPassword('')
        await loadData()
      } else {
        setMessage({ type: 'error', text: json.error || 'Eroare la reinnoirea tokenului' })
      }
    } catch (err) {
  ...
  const handleSync = async (action: string) => {
    ...
    try {
      ...
      if (json.success) {
        setMessage({ type: 'success', text: json.message || `Sincronizare ${action} completa!` })
      } else {
        setMessage({ type: 'error', text: json.message || 'Sincronizare esuata' })
      }

      // Reload data to get updated stats
      await loadData()
    } catch (err) {
      setMessage({ type: 'error', text: 'Eroare la executia sincronizarii' })
      setSyncResult({ success: false, message: 'Eroare conexiune' })
    } finally {
      setSyncing(null)
    }
  }
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      const json = await res.json()
      if (json.success) {
        setMessage({ type: 'success', text: 'Setari salvate cu succes!' })
      } else {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-96">
        ...
        <span className="ml-3 text-lg text-gray-600">Se incarca datele PNI...</span>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">API Furnizori - PNI</h1>
          <p className="text-gray-500 mt-1">Gestionare conexiune, sincronizare si configurare MyPNI B2B</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className={`rounded-xl border-2 p-5 ${
          ...
          {connected && (
            <p className="text-xs text-gray-400 mt-2">
              {data?.apiStatus?.productCount?.toLocaleString()} produse in catalog PNI
            </p>
          )}
        </div>
        ...
        <div className={`rounded-xl border-2 p-5 ${
          ...
          <p className="text-sm text-gray-500 mt-1">
            {data?.tokenInfo?.expiresAt
              ? `Expira: ${formatDate(data.tokenInfo.expiresAt)}`
              : 'Fara informatii expirare'}
          </p>
          <p className="text-xs text-gray-400 mt-1">
            Credentiale: {data?.tokenInfo?.hasCredentials ? 'Configure' : 'Lipsa'}
          </p>
        </div>
        ...
        <div className="rounded-xl border-2 border-indigo-200 bg-indigo-50 p-5">
          ...
          <h3 className="font-semibold text-gray-900">Produse in Baza de Date</h3>
          <div className="mt-2 space-y-1">
            ...
            <p className="text-sm text-gray-600">Fara stoc: <strong className="text-orange-600">{data?.dbStats?.outOfStock?.toLocaleString()}</strong></p>
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
          ...
          Ultimele Sincronizari
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { label: 'Stoc', value: data?.config?.last_sync_stock, color: 'green' },
            { label: 'Preturi', value: data?.config?.last_sync_prices, color: 'blue' },
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-gray-900">Credentiale & Token API</span>
            {!data?.tokenInfo?.hasCredentials && (
              <span className="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full">Necesita configurare</span>
            )}
          </div>
        ...
        {showCredentials && (
          <div className="px-5 pb-5 border-t border-gray-100 pt-4 space-y-4">
            <p className="text-sm text-gray-500">
              Credentiale pentru <strong>b2b.mypni.com</strong> / <strong>b2b.mo.ro</strong>.
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                {refreshingToken ? 'Se reinnoieste...' : 'Reinnoieste Token'}
              </button>
              {data?.tokenInfo?.expiresAt && (
                <span className="text-sm text-gray-500">
                  Expira: {new Date(data.tokenInfo.expiresAt).toLocaleString('ro-RO')}
                </span>
              )}
            </div>
          </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        ...
        <p className="text-sm text-gray-500 mb-4">Porneste sincronizarea selectiv: doar stoc, preturi+stoc, doar imagini, sau import complet</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            {
              id: 'prices', label: 'Preturi + Stoc', desc: 'Actualizare preturi si stoc (3-5 min)',
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  {isRunning && (
                    <span className="flex items-center gap-1 text-xs bg-yellow-100 text-yellow-700 px-2 py-1 rounded-full">
                      <RefreshCw className="w-3 h-3 animate-spin" /> Ruleaza
                    </span>
                  )}
                </div>
                ...
                >
                  {isRunning ? 'Se sincronizeaza...' : 'Porneste'}
                </button>
              </div>
            )
          })}
        ...
        {!connected && (
          <p className="text-sm text-red-500 mt-3 flex items-center gap-2">
            ...
            Conexiunea la API nu este activa. Verifica token-ul si credentialele.
          </p>
        )}
      </div>
        ...
        <div className={`border-2 rounded-xl p-5 ${
          ...
          {(syncResult.updates !== undefined || syncResult.duration !== undefined) && (
            <div className="flex gap-4 text-sm">
              {syncResult.updates !== undefined && <span>Actualizari: <strong>{syncResult.updates}</strong></span>}
              {syncResult.errors !== undefined && syncResult.errors > 0 && <span className="text-red-600">Erori: <strong>{syncResult.errors}</strong></span>}
              {syncResult.duration !== undefined && <span>Durata: <strong>{syncResult.duration}s</strong></span>}
            </div>
          )}
          {syncResult.output && (
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-gray-900">Setari Sincronizare</span>
          </div>
        ...
        {showSettings && (
          <div className="px-5 pb-5 border-t border-gray-100 pt-4 space-y-3">
            {[
              { key: 'sync_prices', label: 'Sincronizeaza Preturi', desc: 'Actualizeaza preturile din API PNI' },
              { key: 'sync_stock', label: 'Sincronizeaza Stoc', desc: 'Actualizeaza cantitatile in stoc' },
              { key: 'sync_images', label: 'Sincronizeaza Imagini', desc: 'Descarca si actualizeaza imaginile produselor' },
              { key: 'sync_descriptions', label: 'Sincronizeaza Descrieri', desc: 'Actualizeaza descrierile produselor' },
              { key: 'sync_specifications', label: 'Sincronizeaza Specificatii', desc: 'Actualizeaza specificatiile tehnice' },
              { key: 'image_overwrite', label: 'Suprascrie Imagini', desc: 'Suprascrie imaginile existente (nu pastreaza copii vechi)' },
            ].map((setting) => (
          ...
          <div className="px-5 pb-5 border-t border-gray-100 pt-4 space-y-4">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              >
                ...
                <option value="0 * * * *">La fiecare ora</option>
              </select>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-sm">Preturi + Stoc</span>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              >
                ...
                <option value="0 3 * * 1">Saptamanal (Luni 3:00)</option>
              </select>
            </div>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-5">
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="font-medium text-gray-600 text-sm">Informatii API PNI</span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs text-gray-500">
          ...
          <div>
            <p className="font-medium text-gray-700">Documentatie</p>
